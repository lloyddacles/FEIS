...
try {
  ...
  $query = "SELECT * FROM evaluation WHERE TC_ID = '".$evaluator."' AND Status = 'Sent'";
  ...
  if ($result = $conn->query($query)) {
    while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      <a href="#" class="btn btn-success" data-role="'.$a.'" data-id="'.$evalid.'">'.$a.'</a>
      </center></td>';
      echo "</tr>";

      echo '<tr id="'.$evalid.'" style="display: none; other-property: value;">';
      echo '<td data-target="evalid">'.$evalid.'</td>';
      echo '<td data-target="teacher">'.$evaluator.'</td>';
      ...
      if (isset($_POST['view'])) {
        ...
        location.href = "T_EvalTableResult.php";
        </script>';
      }
