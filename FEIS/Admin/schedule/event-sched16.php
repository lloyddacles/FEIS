...
$username = "root";
$password = "";
  if (isset($_POST['update_sched16'])) {
try{
  $conn = new PDO("mysql:host=$servername;dbname=feis", $username, $password);
  // set the PDO error mode to exception
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $stc = $_COOKIE['Sched_TC'];
  $query = "SELECT * FROM teaching_load WHERE TC_ID='".$stc."' AND Sched_No=16";
...
if ($result = $conn->query($query)) {
  if ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    $conn->exec($del);
    $update = "UPDATE teaching_load SET Sched_No = Sched_No - 1 WHERE TC_ID = '".$stc."' AND Sched_No BETWEEN 16 AND 20";
    $conn->exec($update);
    echo '<script>
    document.getElementById("teacher").value = "'.$stc.'";
    document.getElementById("aydi").submit();
...
$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$stc = $_COOKIE['Sched_TC'];
$sel = "SELECT * FROM teaching_load WHERE TC_ID='".$stc."' AND Sched_No=16";
          ...
          <div class="column6">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <label for="end"></label>
                  <button class="btn btn-danger btn-lg btn-block" name="delete_sched16">Delete</button>
                </div>
                <div class="col-md-6">
                  <label for="end"></label>
                  <button class="btn btn-info btn-lg btn-block" name="update_sched16">Update</button>
                </div>
